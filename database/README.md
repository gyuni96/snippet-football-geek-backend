...
브리핑과 항목을 함께 가져오는 예시입니다.

```js
const { data: briefing } = await supabase
  .from("briefings")
  .select("*, briefing_items(*)")
  .eq("id", latestBriefing.id)
  .order("sort_order", {
    referencedTable: "briefing_items",
    ascending: true,
  })
  .single();
```

프론트에서는 `briefing_items.item_type`으로 기사 카드와 X 게시물 카드를 나누고, `briefing_items.category`로 카테고리 필터를 구성합니다. 최신순 표시는 일반 소식은 `published_at`, 경기 일정 항목은 `event_at`을 우선해 정렬하면 됩니다.
