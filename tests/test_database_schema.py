...
class DatabaseSchemaTest(unittest.TestCase):
    def test_schema_defines_briefing_tables_and_frontend_fields(self):
        ...
        self.assertIn("create table if not exists public.briefing_articles", content)
        self.assertIn("create table if not exists public.briefing_tweets", content)
        ...
        self.assertIn("tweet_id text not null", content)
        self.assertIn("translated_text_ko text not null", content)
        self.assertIn("llm_provider text not null", content)
        self.assertIn("llm_model text not null", content)
        ...
        self.assertIn("create index if not exists idx_briefing_articles_team_published_at", content)
        self.assertIn("create index if not exists idx_briefing_tweets_team_published_at", content)
        ...
        self.assertIn("alter table public.briefing_articles enable row level security", content)
        self.assertIn("alter table public.briefing_tweets enable row level security", content)
        ...
        self.assertIn("create policy briefing_articles_public_select", content)
        self.assertIn("create policy briefing_tweets_public_select", content)
