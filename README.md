...
Groq 일일 토큰 한도를 보호하기 위해 기본적으로 분당 10회, 실행 1회당 최대 60회까지만 요청하며, 한도 초과 응답이 감지되면 남은 Groq 요청은 즉시 중단합니다. 필요하면 `--groq-requests-per-minute`, `--groq-max-requests` 또는 `.env`의 `GROQ_REQUESTS_PER_MINUTE`, `GROQ_MAX_REQUESTS`로 조정합니다.
