...
- `GROQ_FALLBACK_MODELS`: 기본값 `meta-llama/llama-4-scout-17b-16e-instruct,qwen/qwen3-32b`, 기본 모델 한도 초과 시 사용할 운영 fallback 모델 목록
...
Groq 일일 토큰 한도를 보호하기 위해 기본적으로 분당 10회, 실행 1회당 최대 60회까지만 요청합니다. 기본 모델 한도 초과가 감지되면 같은 실행 안에서 `meta-llama/llama-4-scout-17b-16e-instruct`, `qwen/qwen3-32b` 순서로 fallback합니다. `llama-3.1-8b-instant`는 수동 테스트용 모델로만 사용하고 운영 fallback 목록에는 넣지 않습니다. 필요하면 `--groq-requests-per-minute`, `--groq-max-requests`, `--groq-fallback-model` 또는 `.env`의 `GROQ_REQUESTS_PER_MINUTE`, `GROQ_MAX_REQUESTS`, `GROQ_FALLBACK_MODELS`로 조정합니다.
